...
static void resize_event(gx_voxbassUI *ui);
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	ui->controls[0] = { 0.5, 0.5, 0.0, 1.0, 0.01, 50, 60, 61, 61, false,"VOLUME", KNOB, VOLUME};
...
static void _expose(gx_voxbassUI *ui) {
...
static void controller_expose(gx_voxbassUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void scroll_event(gx_voxbassUI *ui, int direction) {
	float value;
	for (int i=0;i<CONTROLS;i++) {
...
static void button1_event(gx_voxbassUI *ui, double* start_value) {
	for (int i=0;i<CONTROLS;i++) {
		if (aligned(ui->pos_x, ui->pos_y, &ui->controls[i], ui)) {
...
static void set_key_value(gx_voxbassUI *ui, int set_value) {
	float value = 0.0;
	for (int i=0;i<CONTROLS;i++) {
...
static void key_event(gx_voxbassUI *ui, int direction) {
	float value;
	for (int i=0;i<CONTROLS;i++) {
		if (ui->controls[i].is_active) {
			value = min(ui->controls[i].adj.max_value,max(ui->controls[i].adj.min_value, 
...
static void set_previous_controller_active(gx_voxbassUI *ui) {
	for (int i=0;i<CONTROLS;i++) {
...
static void set_next_controller_active(gx_voxbassUI *ui) {
	for (int i=0;i<CONTROLS;i++) {
		if(ui->controls[i].is_active == true) {
			ui->controls[i].is_active = false;
			send_controller_event(ui, i);
...
void get_active_controller(gx_voxbassUI *ui, bool set) {
