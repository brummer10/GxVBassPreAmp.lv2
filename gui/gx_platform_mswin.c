...
void gx_gui_create_window_and_surface(gx_voxbassUI *ui) {
	...
	static TCHAR szClassName[] = TEXT("gx_voxbassUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_destroy_main_window(gx_voxbassUI *ui) {
	...
	UnregisterClass(TEXT("gx_voxbassUIClass"), NULL);
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_voxbassUI *ui = (gx_voxbassUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
