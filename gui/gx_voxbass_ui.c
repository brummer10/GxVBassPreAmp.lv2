...
static void ref_value_changed(GtkAdjustment *adj, gpointer* args) {
	...
	gx_voxbassUI* ui = (gx_voxbassUI*)arg->ui;
    if (ui->block) {
        ui->block = 0;
        return;
    }
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor*   descriptor,
    ...
    *widget = NULL;
    ...
    ui->vlbox = gtk_vbox_new(FALSE, 0);

    ui->pbox = gx_paint_box_new(GTK_ORIENTATION_HORIZONTAL,FALSE, 10);
    set_expose_func(GX_VBASSPREAMP_PAINT_BOX(ui->pbox),"pedal_expose");
    ui->box = gtk_vbox_new(FALSE, 4);
    ui->hbox = gtk_hbox_new(FALSE, 10);
    gtk_container_set_border_width(GTK_CONTAINER(ui->hbox),20);
    ...
    set_expose_func(GX_VBASSPREAMP_PAINT_BOX(ui->pf1box),"frame_expose");
    gtk_container_set_border_width(GTK_CONTAINER(ui->pf1box),10);
    ui->pf2box = gx_paint_box_new(GTK_ORIENTATION_HORIZONTAL,false, 10);
    set_expose_func(GX_VBASSPREAMP_PAINT_BOX(ui->pf2box),"frame_expose");
    gtk_container_set_border_width(GTK_CONTAINER(ui->pf2box),10);
    ui->pf3box = gx_paint_box_new(GTK_ORIENTATION_HORIZONTAL,false, 10);
    set_expose_func(GX_VBASSPREAMP_PAINT_BOX(ui->pf3box),"frame_expose");
    gtk_container_set_border_width(GTK_CONTAINER(ui->pf3box),10);
    ui->pf4box = gx_paint_box_new(GTK_ORIENTATION_HORIZONTAL,false, 10);
    set_expose_func(GX_VBASSPREAMP_PAINT_BOX(ui->pf4box),"frame_expose");
    ...
    ui->label[4] = gtk_label_new("VOLUME");
    ...
    gtk_box_pack_start(GTK_BOX(ui->pf1box), ui->vkbox[4], TRUE, TRUE, 0);
    ...
    ui->args[4]->ui = ui;
    ui->args[4]->port_index = (int)VOLUME;
    ...
    gtk_box_pack_start(GTK_BOX(ui->pf1box), ui->vkbox[0], TRUE, TRUE, 0);
    ...
    gtk_box_pack_start(GTK_BOX(ui->pf2box), ui->vkbox[1], TRUE, TRUE, 0);
    ...
    ui->args[1]->ui = ui;
    ui->args[1]->port_index = (int)CAB;
    g_signal_connect(G_OBJECT(ui->adj[1]), "value-changed",
          G_CALLBACK(ref_value_changed),(gpointer*)ui->args[1]);

    gtk_box_pack_start(GTK_BOX(ui->hbox), ui->pf4box, TRUE, TRUE, 0);
    gtk_box_pack_start(GTK_BOX(ui->vlbox), ui->logo, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(ui->pf4box), ui->vlbox, TRUE, TRUE, 0);
    ...
    gtk_box_pack_start(GTK_BOX(ui->pf3box), ui->bp_box, TRUE, TRUE, 0);
...
static void cleanup(LV2UI_Handle handle) {
    ...
    if (GTK_IS_WIDGET(ui->pbox)) {
        ...
        if (GTK_IS_WIDGET(ui->pf4box)) {
            gtk_widget_destroy(ui->pf4box);
        }
        if (GTK_IS_WIDGET(ui->pbox)) {
