...
#define GTK_VBASSPREAMP_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_VBASSPREAMP_KNOB, GtkVBassPreAmpKnobPrivate))

G_DEFINE_TYPE(GtkVBassPreAmpKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkVBassPreAmpKnobPrivate GtkVBassPreAmpKnobPrivate;

struct _GtkVBassPreAmpKnobPrivate
{
	int knob_x;
	int knob_y;
	int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
	/** check resize **/
	int grow;
 	GtkVBassPreAmpKnobPrivate *priv = GTK_VBASSPREAMP_KNOB_GET_PRIVATE(widget);
	...
	GtkVBassPreAmpKnob *knob = GTK_VBASSPREAMP_KNOB(widget);
...
{
	g_assert(GTK_IS_VBASSPREAMP_KNOB(widget));
	GtkVBassPreAmpKnob *knob = GTK_VBASSPREAMP_KNOB(widget);
	GtkVBassPreAmpKnobPrivate *priv = GTK_VBASSPREAMP_KNOB_GET_PRIVATE(knob);
...
{
	GtkVBassPreAmpKnob *knob = GTK_VBASSPREAMP_KNOB(object);
...
/****************************************************************
 ** init the GtkVBassPreAmpKnobClass
 */

static void gtk_knob_class_init (GtkVBassPreAmpKnobClass *klass)
{
...
//--------- connect the events with funktions
	...
	g_type_class_add_private(obj_class, sizeof (GtkVBassPreAmpKnobPrivate));
}
...
static void gtk_knob_init (GtkVBassPreAmpKnob *knob)
{
...
{
	GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_VBASSPREAMP_KNOB, NULL ));
